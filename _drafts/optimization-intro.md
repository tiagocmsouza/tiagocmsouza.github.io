...
Classical examples include \$l\_{p}\$, the sequence of real numbers \$\\{x\_{1}, x\_{2}, \dots\\}\$ for which \$\sum\_{i=1}^{\infty}{\|x\_{i}\|^p} < \infty\$ and \$0 \leq p < \infty\$. The norm is defined as \$\|\|x\|\|_{p} = \left( \sum\_{i=1}^{\infty}{\|x\_{i}\|^p} \right)^\frac{1}{p}\$ --- the space \$l\_{\infty}\$ is the space of bounded sequences with the norm defined as \$\|\| x \|\|\_{\infty} = \sup\_{i}{\|x\_{i}\|}\$. 

\$L\_{p}[a,b]\$ spaces, for \$1 \leq p < \infty \$, consists of real-valued measurable functions on \$[a,b]\$ for which \$\|x(t)\|^{p}\$ is Lebesgue intergrable and the norm is defined as \$\|\|x\|\|\_{p} = \left( \int\_{a}^{b}{\|x(t)\|^{p} \\, dt} \right)^\frac{1}{p}\$ --- when \$0 \leq p < 1\$ these are metric spaces, whose metric is not induced by a norm. Note that \$\|\|x\|\|\_{p} = 0\$ on this space does not necessarily imply that \$x\$ is the zero vector, as it may be different from zero in a set of measure zero. However, \$L\_{p}[a,b]\$ is a normed linear space if no distinction is made between function equal almost everywhere.
